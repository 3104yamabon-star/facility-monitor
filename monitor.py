...
- 「次の月」遷移の安定化（next_month_selector / has-text / href*='moveCalender' / JS eval）
- 待機は outerHTML 変化検知を優先、補助で月テキスト変化
...
def click_next_month(page,
    ...
    def _try_click(selector: str) -> None:
        nonlocal clicked
        try:
...
def run_monitor() -> None:
    ...
    # dump_html_enabled = bool(dbg.get("dump_calendar_html", True))  # 必要なら使用
