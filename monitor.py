...
def back_to_facility_list(page, selectors: List[str], list_hint: Optional[str]) -> bool:
    """空き状況画面の［もどる］で施設選択画面へ戻す。成功なら True。"""
    def _clicked_and_arrived() -> bool:
        # レース待機後にヒント存在で到達確認
        wait_next_step_ready(page, css_hint=list_hint)
        try:
            return bool(list_hint) and page.locator(list_hint).count() > 0
        except Exception:
            return False
    ...
    for sel in selectors or []:
        try:
            ...
            if el and el.count() > 0:
                if TIMING_VERBOSE:
                    with time_section(f"click back: {sel}"):
    ...
    # 3) 到達できなければ失敗扱い（呼び出し側でフォールバック：従来フロー）
    return False
...
def click_facility_from_list(page, labels: List[str]) -> None:
    ...
    # 一覧は施設名テキストのアンカーなので、既存の try_click_text で十分
    for i, label in enumerate(labels):
